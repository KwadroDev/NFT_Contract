...
const getContract = async(web3) => {
    const response = await fetch("./contracts/LittleHorse.json");
            ...
            if (netId == NETWORK_ID) {
                var awaitContract = async function() {
                    ...
                    if (document.getElementById("price"))
                        document.getElementById("price").textContent = "Price: " + web3.utils.fromWei(NFT_PRICE) + " WETH"
                    if (document.getElementById("presale_price"))
                        document.getElementById("presale_price").textContent = "Presale Price: " + web3.utils.fromWei(PRESALE_PRICE) + " WETH"
                ...
                document.getElementById("web3_message").textContent = "Please connect to Mumbai Testnet";
            }
        });
    };
    awaitWeb3();
...
const setBaseURI = async() => {
    const result = await contract.methods.setBaseURI("https://storage.googleapis.com/nftlilh/")
        .send({ from: accounts[0], gas: 0, value: 0 })
        .on('transactionHash', function(hash) {
            document.getElementById("web3_message").textContent = "Minting...";
        })
...
const setPrice = async() => {
    const result = await contract.methods.setPrice("20000000000000000") //(10000000)
        .send({ from: accounts[0], gas: 0, value: 0 })
        .on('transactionHash', function(hash) {
            document.getElementById("web3_message").textContent = "Minting...";
        })
...
const withdrawTeam = async() => {
    const result = await contract.methods.withdrawTeam("10000000000000000")
        .send({ from: accounts[0], gas: 0, value: 0 })
        .on('transactionHash', function(hash) {
            document.getElementById("web3_message").textContent = "Minting...";
        })
...
const editWhitelistReserved = async() => {
    const result = await contract.methods.editWhitelistReserved(
            [
                ...
                "0xC5bfD7920Cc8152e1c75f35099F01b6d2f32F8A6"
            ], [
                1,
                1
            ]
